...
apoe$apoe_carrier[apoe$apoe=="e2/e2" | apoe$apoe=="e2/e3"] <- "e2"
apoe$apoe_carrier[apoe$apoe=="e3/e3"] <- "e3"
apoe$apoe_carrier[apoe$apoe=="e3/e4" | apoe$apoe=="e4/e4"] <- "e4"
# WMH
wmh <- read.csv('WMH.csv')
colnames(wmh) <- c('id', 'wmh')
# ICV
icv <- read.csv('ICV.csv')
colnames(icv) <- c('id', 'icv')
# triglycerides
trigs <- read.csv('triglycerides.csv')
colnames(trigs) <- c('id', 'trigs')
...
warfarin <- merge(warfarin, apoe, all.x = TRUE)
warfarin <- merge(warfarin, wmh, all.x = TRUE)
warfarin <- merge(warfarin, icv, all.x = TRUE)
warfarin <- merge(warfarin, trigs, all.x = TRUE)
