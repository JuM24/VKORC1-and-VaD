...
load("D:/PhD/NOW/Other/Warfarin/UKB_famhistAD_PCs_unrel_17May2020.RData")
colnames(d1)[which(names(d1) == "IID")] <- "id"
warfarin <- filter(warfarin, id %in% d1$id) # removed 83,835 individuals

# remove id's that may have opted out of the study
opt_out <- read.csv('UK Biobank/Raw files/participant opt-out.csv')
...
mortality <- read.csv("UK Biobank/Raw files/mortality.csv") # mortality data
mortality <- mortality[, 1:2]; colnames(mortality) <- c('id', 'last_date') # filter out the first two columns
mortality$last_date[mortality$last_date==''] <- "2017-09-27" # if they haven't died, set the last sampling day as the "last date"
mortality$last_date <- as.Date(mortality$last_date, "%Y-%m-%d")
...
# export
saveRDS(warfarin, file = 'warfarin_data.rds')
