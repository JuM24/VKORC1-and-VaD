...
icd <- rbind(icd9, icd10)
icd$date <- as.Date(icd$date,"%Y-%m-%d")
...
icd$dem_other <- 0
# icd9
for (c in codes_inpatient$code[codes_inpatient$disorder=='dementia' & codes_inpatient$code_system=='ICD9']){
  icd$dem_other[icd$diagnosis==c & icd$version==9] <- 1 # label diagnosis as present
  codes_inpatient$n[codes_inpatient$code==c] <- length(unique(icd$id[icd$diagnosis==c])) # count the number of occurrences
}
# icd10
for (c in codes_inpatient$code[codes_inpatient$disorder=='dementia' & codes_inpatient$code_system=='ICD10']){
...
icd <- subset(icd, select=c(id, column)) # remove unnecessary columns
...
gp <- subset(gp, select=c(id, data_provider))
inpatient <- distinct(inpatient, id, .keep_all = TRUE)
inpatient <- subset(inpatient, select=c(id, column))
...
diagnoses$data_provider <- NULL
diagnoses$column <- NULL
